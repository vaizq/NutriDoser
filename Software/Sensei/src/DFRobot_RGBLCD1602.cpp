...
void DFRobot_RGBLCD1602::init()
{
    ...
    ESP_ERROR_CHECK(i2c_master_bus_add_device(i2c::bus, &cfg, &lcd));

    cfg.device_address = _RGBAddr;

    ESP_ERROR_CHECK(i2c_master_bus_add_device(i2c::bus, &cfg, &rgb));
...
{
    i2c_master_transmit(lcd, data, len, -1);
    /*
...
{
    const uint8_t buf[2] = {addr, data};
    i2c_master_transmit(rgb, buf, 2, -1);
